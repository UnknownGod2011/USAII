  ...
  try {
    ...
    const created = await getDb().founderIntake.create({
      data: {
        userId: user.id,
        name: intake.name,
        locationCountry: intake.locationCountry,
        locationCity: intake.locationCity,
        status: intake.status,
        hoursPerWeek: intake.hoursPerWeek,
        budget: intake.budget,
        skillsJson: JSON.stringify(intake.skills),
        ...
        transcriptJson: JSON.stringify(intake.transcript),
        skippedOrUnclearJson: JSON.stringify(intake.skippedOrUnclearFields),
        answerValidations: {
          create: intake.answerValidations.map((validation) => ({
            questionId: validation.questionId,
            originalQuestion: validation.originalQuestion,
            userAnswer: validation.userAnswer,
            expectedField: validation.expectedField,
            isUsable: validation.isUsable,
            qualityScore: validation.qualityScore,
            extractedValueJson: JSON.stringify(validation.extractedValue),
            issuesJson: JSON.stringify(validation.issues),
    ...
    return NextResponse.json({ error: error instanceof Error && error.message === "UNAUTHORIZED" ? "Unauthorized" : "Could not save intake" }, { status: error instanceof Error && error.message === "UNAUTHORIZED" ? 401 : 500 });
  }
}
