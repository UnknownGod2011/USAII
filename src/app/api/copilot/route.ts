  ...
  try {
    ...
    if (productionBlobStateEnabled()) {
      ...
      const history = state.chatMessages
        .filter((message) => message.startupIdeaId === idea.id && ["user", "assistant"].includes(message.role))
        .sort((a, b) => a.createdAt.localeCompare(b.createdAt))
        .slice(-8)
        .map((message) => ({ role: message.role as "user" | "assistant", content: message.content }));
      const { answer, mode } = await answerCopilotQuestion(question, brief, history);
      const now = nowIso();
      ...
      return NextResponse.json({ answer, references: relevantReferences(question, brief), mode });
    }
    ...
    if (!idea || !saved) return NextResponse.json({ answer: "Finish and approve an evidence-reviewed direction first.", references: [] }, { status: 409 });
    const brief = (JSON.parse(saved.contentJson) as { brief: LaunchBrief }).brief;
    const priorMessages = await getDb().chatMessage.findMany({
      where: { userId: user.id, startupIdeaId: idea.id, role: { in: ["user", "assistant"] } },
      orderBy: { createdAt: "desc" },
      take: 8,
    });
    const history = priorMessages.reverse().map((message) => ({
