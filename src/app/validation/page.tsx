        ...
        <section className="mx-auto max-w-6xl px-5 pb-16 pt-6">
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <section className="terminal-card flex flex-col items-start justify-between gap-5 p-6 md:flex-row md:items-center"><div><p className="mono-label">Ready for the workspace</p><h2 className="mt-2 text-xl font-semibold">{weak ? "Proceed with the finalized direction and keep the evidence gaps visible." : "Turn this validated direction into a complete Launch Brief."}</h2>{error && <p className="mt-2 text-sm text-red-300">{error}</p>}</div><button onClick={approve} disabled={approving} className="btn-primary shrink-0">{approving ? "Preparing Launch Brief…" : "Run In-Depth Market Research"} <ArrowRight className="h-4 w-4" /></button></section>
